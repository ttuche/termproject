...
{ 
 ...
 //       time_sleep(0.1);
    ...
    // 0 오른쪽 
        if( error_R>2.5)
           set_PWM_dutycycle(pi,PINN0,15); 
           set_PWM_dutycycle(pi,PINN1,20);

        if( error_L>2.5)
           set_PWM_dutycycle(pi,PINN0,18);
    
           set_PWM_dutycycle(pi,PINN1,15);
    }*/
    if(distance2 >10){
      
     //   forward();
      
        set_PWM_dutycycle(pi,PINN1,34);
        set_PWM_dutycycle(pi,PINN0,30); 
    
        if(error_L>3){ 
           set_PWM_dutycycle(pi,PINN1,20); 
           set_PWM_dutycycle(pi,PINN0,24); 
        }else if(error_R>3){

           set_PWM_dutycycle(pi,PINN1,32); 
           set_PWM_dutycycle(pi,PINN0,25); 
        }/*else if(error_L>3 && error_R>3){
          stop();
        }*/
    }
...
void forward(void)
{
       set_PWM_dutycycle(pi, PINN0,f_right); //38
       set_PWM_dutycycle(pi, PINN1,f_left);  //40
